...
namespace Invector.vMelee
{
    ...
    public class vMeleeAttackObject : vMonoBehaviour
    {
        ...
        public virtual void OnHit(vHitBox hitBox, Collider other)
        {
            ...
            {
                //Set Target for command sequence
                // p.target=other.gameObject.GetComponent<Entity>();
                //
                var inDamage = false;
                var inRecoil = false;
                ...
                if (inDamage || inRecoil)
                {
                    ...
                    if (inDamage == true)
                    {
                        ...
                        FindObjectOfType<CombatTextSystem>().GetComponent<CombatTextSystem>().CreateCombatText(hitBox.attackObject.damage.damageValue,transform.position,
                        false,false,true);
