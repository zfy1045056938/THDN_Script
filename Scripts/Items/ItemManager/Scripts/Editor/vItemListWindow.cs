...
namespace Invector.vItemManager
{
    public class vItemListWindow : EditorWindow
    {
        ...
        static THDNGameDatabase tHDNGameDatabase;
		...
		// tHDNGameDatabase = (THDNGameDatabase)Resources.Load("THDNGameDatabase", typeof(THDNGameDatabase)) as THDNGameDatabase;
        ...
        public virtual void OnGUI()
        {
            ...
            if (OnSelectItem == null)
            {
                ...
                if (!inAddItem && GUILayout.Button("Create New Item"))
                {
                    ...
                    if(GUILayout.Button("LoadAllItems")){
                            GDELoadDEs();
                    }
                    if(GUILayout.Button("Clear Old Items")){
                        ClearItems();
                    }
                    if(GUILayout.Button("Load Craft item")){
                        LoadAllCraftItem();
                    }
  ...
  Debug.Log("Start Clear");
	...
	private void GDELoadDEs()
	{

       itemDatabase.itemList = new List<vItem>();
		List<GDEItemsData> gde = GDEDataManager.GetAllItems<GDEItemsData>();

		//try got every item data at xlxs 
		for (int i = 0; i < gde.Count; i++)
		{
			GDEItemsData item = new GDEItemsData(gde[i].Key);   //details
			//Create ScriptObject
			ScriptableObject.CreateInstance<ScriptableItem>();
			//Bind Data
			vItem s = new vItem()
			{
				itemID = item.ItemID,
                ...
                //TODO
				// icon = Util.CreateSprite(item.ItemSprite),
                icon=null,
                iconName = item.ItemSprite,
                originalObject = item.ItemObj,
                dropObject = item.ItemObj,

				itemRarity = Util.Convert_ItemRarity(item.ItemRarity),
				type = Util.Convert_ItemType(item.ItemType),
				...
				itemAShield = item.ItemArmor,
				critPerc = item.ItemCritPerc,
				blockPerc = item.ItemBlockPerc,
				flashPerc = item.ItemFlashPerc,
				itemPrice = item.Price,
				stackable = item.Stackable,
                maxStack = item.MaxStaksize,
                //different canCraft
                canCraft =item.CanCraft,
                
				// itemScript = item.ItemScript,
				effectType = Util.Convert_ItemExtraEffect(item.ItemExtraEffect),
				effectAmount=item.ItemExtraAmount,
                //
                // itemsToCraft = LoadCraftItems(item.ItemName),


			};
            //Changelogs set craftItems 
            

			//
			// AssetDatabase.CreateAsset(s, "Assets/Resources/Items/" + s.name + ".asset");
			// AssetDatabase.SaveAssets();
			// AssetDatabase.Refresh();
            //
            Debug.Log("Add Item to resources"+item.ItemName.ToString());
            AddItem(s);
			//Save database und resources
			s.itemID = i;
			itemDatabase.itemList.Add(s);
			//
    ...
    //got all item then set craft item for CanCraft foreach itemList
      
        
    }

    //Load item by id then load all crafitem
    // craftitem -> vitem 
    public List<vItemCraft> LoadCraftItems(string itemName){
       Debug.Log("Try load craftItem in itemList");
        var  cItemList= new List<vItemCraft>();

    List<GDECraftItemsData> craftList= GDEDataManager.GetAllItems<GDECraftItemsData>();
...
//TODO
    public void LoadAllCraftItem(){
        Debug.Log("Start Load All CI");
          if(itemDatabase.itemList.Count>0){
            for(int j=0;j<itemDatabase.itemList.Count;j++){
                if(itemDatabase.itemList[j].canCraft==true){
                    //set item amount und name to itemCraft
                    itemDatabase.itemList[j].itemsToCraft =LoadCraftItems(itemDatabase.itemList[j].name);
        ...
        private void AddItem(vItem item)
        {
            ...
            if (item && !itemList.items.Find(it => it.name.Equals(item.name)))
            {
