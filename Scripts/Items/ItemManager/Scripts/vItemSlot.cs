...
namespace Invector.vItemManager
{
    ...
    public vItemSlot(){}
        public vItemSlot(ItemReference item1, int itemStacksize=1)
        {
            this.items = item1;
            this.amount = itemStacksize;
        }
        ...
        protected virtual void ChangeDisplayIcon(vItem item)
        {
            if (icon && item)
            {
                icon.sprite = Util.CreateSprite(item.iconName);
                color.a = 1;
                icon.color = color;
            }
        }
