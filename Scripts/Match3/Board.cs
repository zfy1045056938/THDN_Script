...
/// 105  For Digger Module at denCave ,when player use EP diggertools start matches module
/// For GP includes 
/// 1.Time Limit: player start the matches until the time ends,when the score higher than matrix required,then got successful.

/// 
/// </summary>


///Declare Matrix Stone Config
    public class MatrixStone{
 ...
 public int width;
   ...
   public bool m_playerInputEnabled = true;
    ...
    public void SetupBoard()
    {
        ...
        {
            Debug.LogError("Can't Got Entity");
        }
    }
    ...
    /// <returns></returns>
    public bool CheckEntity(Players player,Entity monster)
    {
    ...
    IEnumerator SwitchTilesRoutine(Tile clickedTile, Tile targetTile)
    {
        if (m_playerInputEnabled)
        {
            ...
            if (targetPiece != null && clickedPiece != null)
            {
                ...
                // {
                //     clickedPiece.GPType = targetPiece.GPType;
                //     colorMatches = FindAllGPType(clickedPiece.GPType);
                // }
                // else if (!IsColorBomb(clickedPiece) && IsColorBomb(targetPiece))
                // {
                //     targetPiece.GPType = clickedPiece.GPType;
                //     colorMatches = FindAllGPType(targetPiece.GPType);
                // }
                // else if (IsColorBomb(clickedPiece) && IsColorBomb(targetPiece))
                // {
   ...
   public bool IsNextTo(Tile start, Tile end)
    ...
    List<GamePieces> FindMatches(int startX, int startY, Vector2 searchDirection, int minLength = 3)
    {
        ...
        for (int i = 1; i < maxValue - 1; i++)
        {
            ...
            {
                if (nextPiece.matchValue == startPiece.matchValue && !matches.Contains(nextPiece) && nextPiece.matchValue != GPType.None)
                {
    ...
    #region Highlight Module
    //highlight suitable for skill, when Matches Collect energy tile enough to use
    // skill , highlight skill pieces und noice with highlight und player at the BF 
    // Check the buff und use skill, then COOLDOWN for skill 
...
//                piece.ScorePoints(m_scoreMultiplier, bonus);
                ...
                if (p >= 3)
                {
                    //TODO Add tile to Collection Pool
                    //CollectPool
                    switch (piece.matchValue)
                    {
                        //TODO Collect by pieces
                        case GPType.STR:
   ...
   public void ClearAndRefillBoard(List<GamePieces> GamePiecess)
    ...
    IEnumerator ClearAndRefillBoardRoutine(List<GamePieces> GamePiecess)
    {
        ...
        do
        {
            ...
            //Auto Atk sequence
            yield return StartCoroutine(AutoAtkRoutine());
            // add pause here 
            yield return null;
    ...
    private IEnumerator AutoAtkRoutine()
    {
    ...
    //When matches und refill board includes these rules und refill
    //1.at least 2 pre-matches gp 
    //2.when without matches then refillboard(    IEnumerator ClearAndCollapseRoutine(List<GamePieces> GamePiecess)
    public IEnumerator ClearAndCollapseRoutine(List<GamePieces>  GamePiecess){
        List<GamePieces> movingPieces = new List<GamePieces>();
        List<GamePieces> matches = new List<GamePieces>();
...
//TODO Remove
    ...
    List<GamePieces> FindAllGPType(GPType mValue)
    {
