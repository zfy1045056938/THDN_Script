...
public class DeckBuilder:MonoBehaviour{
    ...
    public void BuildADeckFor(CharacterAsset asset){
        ...
        Debug.Log(asset.atkNum.ToString()+"&&"+asset.defNum.ToString());
 ...
 var cc =CardCollection.instance.GetCardAssetByName("攻击");
                 var dc=CardCollection.instance.GetCardAssetByName("防御力");
            while (damageCard > 0) {
                //add damage card
                //as spell card
               Debug.Log("damagecard"+damageCard);
                 if(!ribbons.ContainsKey(cc)){
                
                GameObject common = Instantiate(CardNamePrefab,Content) as GameObject ;
                
               CardNameRibbon ribbon = common.GetComponent<CardNameRibbon>();
               common.GetComponent<Button>().interactable=false;
                ribbon.ApplyAsset(cc, 1);
               ribbons.Add(cc, ribbon);
                }else{
                    ribbons[cc].SetQuantity(1);
                }
                deckList.Add(cc);
                 damageCard--;
            ...
            while (armorCard > 0)
            {
                Debug.Log(armorCard+"armorcard");
         if(!ribbons.ContainsKey(dc)){
                GameObject common = Instantiate(CardNamePrefab, Content) as GameObject;
     CardNameRibbon ribbon = common.GetComponent<CardNameRibbon>();
     common.GetComponent<Button>().interactable=false;
                ribbon.ApplyAsset(dc, 1);
                // ribbons.Add(cc, ribbon);
                
                ribbons.Add(dc, ribbon);
                }else{
                    ribbons[dc].SetQuantity(1);
                }
                armorCard--;
