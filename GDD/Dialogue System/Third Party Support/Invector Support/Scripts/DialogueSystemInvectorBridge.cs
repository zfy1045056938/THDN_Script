...
namespace PixelCrushers.DialogueSystem.InvectorSupport
{
    ...
    public class DialogueSystemInvectorBridge : MonoBehaviour
    {
        ...
        private void RegisterLuaFunctions()
        {
            ...
            //Add-on
         //Skill Module
             Lua.RegisterFunction("AddPersonalSKill", this, SymbolExtensions.GetMethodInfo(() => AddPersonalSKill(string.Empty(),(double)0)));
            Lua.RegisterFunction("CheckPersonalSkill", this, SymbolExtensions.GetMethodInfo(() => CheckPersonalSkill(string.Empty(),(double)0)));
         
         Lua.RegisterFunction("AddReward", this, SymbolExtensions.GetMethodInfo(() => AddReward((double)0),(double)0,(double)0));
        ...
        public void AddPersonalSKill(string skill,double s){

        }
        
        public void CheckPersonalSKill(string skill,double s){
            var p =Players.localPlayer;
            if(p){
            if(skill=="Cheat"){
                if(p.cheat > (int)s){
                    DialogueLua.SetVariable("CheckSkillSuccess","success");
                }else{
                     DialogueLua.SetVariable("CheckSkillSuccess","success");
                }
        ...
        public void AddReward(double exp,double money,double dust){

            var p=Players.localPlayer;
            if(p){
                p.CmdAddReward((int)exp,(int)money,(int)dust);
            }
        }

        public void MatchesModule(){
            var gs = FindObjectOfType<GlobalSetting>();
            if(gs){
                gs.MatchesModule();
            }
        }
